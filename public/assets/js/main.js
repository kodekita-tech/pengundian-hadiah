const initAppToggler = () => {
    ...
    appMenubars.addEventListener("mouseenter", () => {
        if (
            document.documentElement.getAttribute("data-app-sidebar") === "mini"
        ) {
            document.documentElement.setAttribute(
                "data-app-sidebar",
                "mini-hover"
...
const saerchList = () => {
    ...
    $.getJSON("assets/ajax/search.json", function (data) {
        ...
        // File tidak ada, gunakan array kosong
        listItems = [];
    });
    ...
    jQuery(
        ...
        .on("click", function (e) {
            if (jQuery(this).hasClass("open")) {
                ...
                if (
                    jQuery(this).parent("li").children(".menu-inner").length > 0
                ) {
                    e.preventDefault();
    ...
    const showActiveTheme = (theme, focus = false) => {
        ...
        if (selectedIcon) {
            activeThemeIcon.className =
                selectedIcon.className + " theme-icon-active";
        }
