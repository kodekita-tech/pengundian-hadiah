    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h6 class="card-title mb-0">Create Event</h6>
            </div>
            <div class="card-body">
                <form action="{{ route('admin.event.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('nm_event') is-invalid @enderror" 
                               id="nm_event" name="nm_event" value="{{ old('nm_event') }}" required>
                        @error('nm_event')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('opd_id') is-invalid @enderror" 
                            ...
                            @if(old('opd_id'))
                                @php
                                    $selectedOpd = \App\Models\Opd::find(old('opd_id'));
                                @endphp
                                @if($selectedOpd)
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" 
                            ...
                            <option value="pengundian" {{ old('status') == 'pengundian' ? 'selected' : '' }}>Pengundian</option>
                            <option value="selesai" {{ old('status') == 'selesai' ? 'selected' : '' }}>Selesai</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                        <small class="form-text text-muted">Status "Draft" akan membuat event tanpa membuka pendaftaran</small>
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('tgl_mulai') is-invalid @enderror" 
                                   id="tgl_mulai" name="tgl_mulai" value="{{ old('tgl_mulai') }}" placeholder="Pilih tanggal & jam">
                            @error('tgl_mulai')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('tgl_selesai') is-invalid @enderror" 
                                   id="tgl_selesai" name="tgl_selesai" value="{{ old('tgl_selesai') }}" placeholder="Pilih tanggal & jam">
                            @error('tgl_selesai')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="d-flex justify-content-between">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="fi fi-rr-check me-1"></i> Simpan Event
                        </button>
                    </div>
                </form>
            </div>
